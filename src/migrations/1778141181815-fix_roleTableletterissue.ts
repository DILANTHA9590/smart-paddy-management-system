...
export class FixRoleTableletterissue1778141181815 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`roles\` CHANGE \`role_name\` \`roleName\` varchar(255) NOT NULL`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`roles\` CHANGE \`roleName\` \`role_name\` varchar(255) NOT NULL`,
    );
    await queryRunner.query(
      `CREATE UNIQUE INDEX \`IDX_ac35f51a0f17e3e1fe12112603\` ON \`roles\` (\`role_name\`)`,
    );
  }
}
