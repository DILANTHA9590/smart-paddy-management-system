...
export class AddRoleUserFarmerRelations1781282344883 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`farmers\` (\`id\` int NOT NULL AUTO_INCREMENT, \`nic\` varchar(255) NOT NULL, \`phone_number\` varchar(255) NOT NULL, \`address\` text NOT NULL, \`district\` varchar(255) NOT NULL, \`province\` varchar(255) NOT NULL, \`village\` varchar(255) NOT NULL, \`date_of_birth\` date NOT NULL, \`gender\` enum ('Male', 'Female', 'Other') NOT NULL, \`profile_image\` varchar(255) NULL, \`organization_id\` int NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`user_id\` varchar(36) NULL, UNIQUE INDEX \`IDX_2a96fbb368a1162f96481339be\` (\`nic\`), UNIQUE INDEX \`REL_f2598912e9b3fd889f6fecee93\` (\`user_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`ALTER TABLE \`roles\` ADD \`description\` varchar(255) NULL`);
        await queryRunner.query(`ALTER TABLE \`roles\` ADD \`updatedBy\` varchar(255) NULL`);
        ...
        await queryRunner.query(`ALTER TABLE \`roles\` ADD \`roleName\` enum ('SUPER_ADMIN', 'ADMIN', 'FARMER', 'AGRICULTURE_ADVISOR', 'IRRIGATION_OFFICER', 'SUPERVISOR', 'ORGANIZATION_MANAGER') NOT NULL`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE UNIQUE INDEX \`IDX_992f24b9d80eb1312440ca577f\` ON \`roles\` (\`roleName\`)`);
        await queryRunner.query(`ALTER TABLE \`roles\` DROP COLUMN \`id\``);
        await queryRunner.query(`ALTER TABLE \`roles\` ADD \`id\` int NOT NULL AUTO_INCREMENT`);
